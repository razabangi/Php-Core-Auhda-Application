<?php
session_start();
if($_SESSION['user_name'] == "")
{
  header("location:admin_login.php");
}

include("db/connection.php");

if(isset($_GET['id']))
{
$edit = $_GET['id'];

$query = $con->prepare("select * from tbl_order where id = '$edit'");
$query->setFetchMode(PDO::FETCH_ASSOC);
$query->execute();
$fetch = $query->fetch();
}
?>

<?php include('partials/header.php'); ?>

<?php include('header/customer.nav.php'); ?>


  <!-- end/row -->
</div>
<!-- /end-container-here -->
  </div>
</section><!-- /main-inventory-search sec close -->



<section class="main-content-inven-add">
  <!-- /main-content-inven-add-start -->
<div class="container">
  <!-- container-start -->
<div class="row">
  <!-- row-start -->
<div class="col-sm-12">
<div id="Page_wrap">

      <div class="head-top">
      
        <div class="row"> <!-- start/row -->

<!-- end/col-sm-6 -->
</div>
<ul class="nav navbar-nav navbar-left mynave">
  <li><a href="#"><p class="main-heading22"><strong>Order Tracking &nbsp;</strong><span>/&nbsp;New</span></p>
</a></li>
        <li><a href="#"><button class="btn btn-success my-new-btn12">Save</button></a></li>
        <li><a href="#"><button class="btn btn-danger  my-new-btn12">Discard</button></a></li>
    </ul>

      <ul class="nav navbar-nav navbar-right mynave">
        <li><a href="#"><span class="fa fa-trash"></span>Active</a></li>
        <li><a href="#"><span class="fa fa-pencil-square-o"></span>0.00 Invoiced</a></li>
        <li><a href="#"><span class="fa fa fa-usd"></span>0.00 Sales</a></li>
       </ul>
   </div>



<div class="col-sm-8">
  <form action="update_orderTracking.php" method="POST">
<div class="form-group"> 

<input type="hidden" name="id" value="<?php echo $fetch['id']; ?>">

 <input type="text" name="client_name" value="<?php echo $fetch['client_name']; ?>" id="client_name" placeholder="Client Name" class="myform-control-s other">
 </div>


<div class="form-group">
 <input type="text" name="order_no" value="<?php echo $fetch['order_no']; ?>" id="order_no" placeholder="Order No" class="myform-control-d">
</div>


</div>

<div class="col-sm-4">  
<div class="pull-right" data-original-title="" title="">
<a href="#">
<img border="1" name="image" src="assets/images/placeholder.png" style="width: 90px; height: 90px; border:1px solid #ccc">
   </a>     
            
        
    </div>




</div>

<!-- <div class="container mb-50"> -->
<div class="row">
<div class="col-sm-6">
<div class="form-group">
<div class="col-sm-3"><label class="newlabels">Entry Date</label><br><label class="newlabels">Final Date</label><br><label class="newlabels">Resume Id</label></div>
<div class="col-sm-9">
 <input type="date" name="date_of_order" value="<?php echo $fetch['date_of_order']; ?>" id="date_of_order" placeholder="Date of Entry" class="myform-control-d">

  <input type="date" name="final_delivery_date" value="<?php echo $fetch['finally_delivery_date']; ?>" id="final_delivery_date" placeholder="Final Delivery Date" class="myform-control-d">

 <input type="text" name="resume_id" value="<?php echo $fetch['resume_id']; ?>" id="resume_id" placeholder="Resume Id" class="myform-control-d">


    </div>
  </div>
</div>


  <div class="col-sm-6">
<div class="form-group">
<div class="col-sm-3"><label class="newlabels">Required</label><br><label class="newlabels">Sample Status</label><br><label class="newlabels">Resume Status</label></div>
<div class="col-sm-9">

<input type="text" name="sample_required" value="<?php echo $fetch['sample_required']; ?>" id="sample_required"  placeholder="Sample Status" class="myform-control-d">

<input type="text" name="sample_status" value="<?php echo $fetch['sample_status']; ?>" id="sample_status"  placeholder="Sample Status" class="myform-control-d">

  <input type="text" name="full_resume_status" value="<?php echo $fetch['full_resume_made']; ?>" name="full_resume_status" placeholder="Resume Status"  class="myform-control-d">

</div>
<!-- <div class="col-sm-3"><label>Postion</label><br><label>Profession</label></div> -->

  

    </div>
  </div>

</div>




<br><br>

<div class="container">
<input type="submit" name="update" class="btn btn-success" value="Update">
</div>

</form>



</div>
<!-- row close -->
</div>
</div>
<!-- main-content-inven-add/end -->
</section>









<?php include('partials/footer.php'); ?>

